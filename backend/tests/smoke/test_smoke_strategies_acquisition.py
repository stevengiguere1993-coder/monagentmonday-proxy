...
def test_residentiel_prime_et_metriques():
    """Ratio > 80 % → prime d'assurance prêt financée (paiement sur le
    prêt total) ; RNO, couverture de la dette, cashflow par porte,
    capital an 1, rendement total exposés et cohérents."""
    from app.services.lead_analysis_finance import pmt_canadian, solde_pret_canadien

    r = compute_all(
        _inputs(strategie="residentiel", chantier_actif=True, ltv_residentiel=0.90),
        use_aph_select=False,
    ).to_dict()["residentiel"]
    assert r["pret_retenu"] == 900_000.0
    assert r["taux_prime"] == 0.031
    assert abs(r["prime_assurance"] - 900_000 * 0.031) < 0.01
    assert abs(r["pret_total"] - 900_000 * 1.031) < 0.01
    assert abs(r["paiement_mensuel"] - pmt_canadian(0.04, 300, 900_000 * 1.031)) < 0.01
    # La prime est financée : le cash à sortir ne bouge pas (100 k + frais).
    assert r["detail_mdf"]["mdf_brute"] == 100_000.0
    assert abs(r["rno_actuel"] - (100_000 - 14_800)) < 0.01
    assert abs(r["dscr_actuel"] - r["rno_actuel"] / r["hypotheque_annuelle"]) < 1e-3
    assert abs(r["cashflow_porte_mois_actuel"] - r["cashflow_actuel"] / 10 / 12) < 0.01
    cap1 = 900_000 * 1.031 - solde_pret_canadien(900_000 * 1.031, 0.04, 25, 1)
    assert abs(r["capital_rembourse_an1"] - cap1) < 0.01
    p1 = r["projection"][1]
    assert abs(p1["capital_annuel"] - cap1) < 0.02
    assert abs(p1["cashflow_plus_capital"] - (p1["cashflow"] + p1["capital_annuel"])) < 0.02
    assert abs(p1["rno"] - (p1["revenus"] - p1["depenses"])) < 0.02
    assert abs(
        r["rendement_total_an1_optimise"]
        - (r["cashflow_optimise"] + cap1) / r["mdf_cash"]
    ) < 1e-3
    # Ratio ≤ 80 % : pas de prime.
    r80 = compute_all(
        _inputs(strategie="residentiel", chantier_actif=True),
        use_aph_select=False,
    ).to_dict()["residentiel"]
    assert r80["prime_assurance"] == 0.0 and r80["pret_total"] == 800_000.0
